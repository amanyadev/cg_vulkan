...
void VulkanApp::run() {
    ...
    m_windowManager = std::make_unique<WindowManager>(WIDTH, HEIGHT, "Vulkan glTF PBR Viewer");
    initVulkan();
    mainLoop();
    cleanup();
...
void VulkanApp::initVulkan() {
    ...
    std::cout << "Creating glTF Viewer..." << std::endl;
    m_viewer = std::make_unique<GLTFViewer>(m_device.get(), m_swapChain.get());
    m_viewer->initialize();
    
    // Connect input callbacks to the viewer
    setupInputCallbacks();
    
    // No default model loaded - users can drag & drop models
    ...
    std::cout << "  2 : Wireframe rendering" << std::endl;
    std::cout << "  3 : Points rendering" << std::endl;
    std::cout << "  A : Toggle auto-rotation" << std::endl;
    std::cout << "  G : Toggle gizmo" << std::endl;
    std::cout << "  W : Toggle wireframe overlay" << std::endl;
    std::cout << "  B : Toggle bounding box" << std::endl;
    std::cout << "Drag & Drop:" << std::endl;
    std::cout << "  Drop .gltf/.glb files to load models" << std::endl;
    std::cout << "  Drop .exr files (HDR textures supported)" << std::endl;
    std::cout << "================" << std::endl;
}
...
void VulkanApp::mainLoop() {
    ...
    while (!m_windowManager->shouldClose()) {
        ...
        // Update UI
        m_debugUI->newFrame();
        m_debugUI->renderViewerPanel(m_performanceStats, m_viewer.get(), m_backgroundSettings);
        m_debugUI->render();
        ...
        // Render viewer (updates uniform buffers)
        m_viewer->render();
...
void VulkanApp::drawFrame() {
    ...
    *m_sync->getImageInFlightFence(imageIndex) = m_sync->getInFlightFence(currentFrame);
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || m_windowManager->wasResized()) {
...
void VulkanApp::recreateSwapChain() {
    ...
    // Recreate swap chain and pipeline
    m_swapChain = std::make_unique<SwapChain>(m_device.get(), VkExtent2D{static_cast<uint32_t>(width), static_cast<uint32_t>(height)});
    m_pipeline = std::make_unique<GraphicsPipeline>(m_device.get(), m_swapChain.get());
}
...
void VulkanApp::updatePerformanceStats() {
    ...
    // No longer needed for glTF viewer
}

void VulkanApp::processInput() {
    // Input handling is now done by GLTFViewer through callbacks
}

void VulkanApp::setupInputCallbacks() {
    if (!m_windowManager || !m_viewer) return;
    
    // Connect mouse movement
    m_windowManager->setMouseMoveCallback([this](double x, double y) {
        this->onMouseMove(x, y);
    });
    
    // Connect mouse buttons
    m_windowManager->setMouseButtonCallback([this](int button, int action, int mods) {
        this->onMouseButton(button, action, mods);
    });
    
    // Connect scroll
    m_windowManager->setScrollCallback([this](double xoffset, double yoffset) {
        this->onScroll(xoffset, yoffset);
    });
    
    // Connect keyboard
    m_windowManager->setKeyCallback([this](int key, int scancode, int action, int mods) {
        this->onKey(key, scancode, action, mods);
    });
    
    // Connect file drop
    m_windowManager->setDropCallback([this](int count, const char** paths) {
        this->onDrop(count, paths);
    });
}

void VulkanApp::onMouseMove(double xpos, double ypos) {
    if (m_viewer) {
        m_viewer->onMouseMove(xpos, ypos);
    }
}

void VulkanApp::onMouseButton(int button, int action, int mods) {
    if (m_viewer) {
        m_viewer->onMouseButton(button, action, mods);
    }
}

void VulkanApp::onScroll(double xoffset, double yoffset) {
    if (m_viewer) {
        m_viewer->onScroll(xoffset, yoffset);
    }
}
...
void VulkanApp::onDrop(int count, const char** paths) {
    if (m_viewer && count > 0) {
        ...
        if (ext == "gltf" || ext == "glb") {
            m_viewer->loadModel(filePath);
        } else if (ext == "exr") {
            std::cout << "EXR file dropped: " << filePath << std::endl;
            std::cout << "EXR files are supported as textures in glTF models" << std::endl;
            // EXR files are handled as textures when referenced in glTF models
        } else {
            std::cout << "Unsupported file type: " << ext << std::endl;
            std::cout << "Supported formats: .gltf, .glb, .exr (as textures)" << std::endl;
        }
    }
}
